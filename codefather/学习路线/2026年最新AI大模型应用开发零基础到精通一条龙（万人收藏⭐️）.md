# 2026年最新AI大模型应用开发零基础到精通一条龙（万人收藏⭐️）
...
AI 大模型应用开发求职高频面试题：[开始刷题](https://www.mianshiya.com/bank/1906189461556076546)
...
学习过程中，推荐使用 [AI 资源导航](https://ai.codefather.cn/) 中的工具辅助学习，用 AI 学 AI 会更高效。



### 学习路线图

![AI 大模型应用开发学习路线思维导图](https://pic.yupi.icu/roadmap/ai-llm-application-development-roadmap.png)
...
2）如何理解大模型和通用人工智能（AGI）之间的关系？它们在目标和实现上有何区别？
...
- [NLP](https://zh.wikipedia.org/zh-hans/自然语言处理)基础（自然语言处理）。关于 NLP 自然语言处理的详细学习，可以查看 [NLP 自然语言处理学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748600665157634)
  - 数学基础
    - 线性代数
    - 概率统计
    - 微积分
    - 最优化
  - [机器学习](https://zh.wikipedia.org/zh-hans/机器学习)。关于机器学习的详细学习，可以查看 [机器学习学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748464077647873)
    - TF-IDF（词频-逆文档频率）
    - 朴素贝叶斯
    - SVM在文本分类中的应用
    - 线性回归
    ...
    - 逻辑回归
    - Softmax
  - 神经网络基础。关于深度学习的详细学习，可以查看 [深度学习学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748511389396993)
    - RNN/LSTM（时序建模）
    - CNN（文本分类）
    - Optimizer优化器
    - PyTorch 深度学习框架
...
学习AI 免不了要跟Python打交道，因为AI的很多库和框架都是Python写的，所以我们首先要学好Python。关于 Python 的详细学习，可以查看 [Python 学习路线](https://www.codefather.cn/course/1789189862986850306/section/1789190283176419330)

对于提示词工程，这部分其实是大模型应用中一个很有意思的环节。你需要先搞清楚什么叫Prompt，了解它的概念、作用，以及如何通过设计有效的提示词来引导大模型生成期望的输出。关于 Prompt Engineering 提示词工程的详细学习，可以查看 [Prompt Engineering 提示词工程学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748694684676098)

然后来再一步步试着调试和优化Prompt的效果，这个过程很大程度上依赖于不断地实践和尝试，所以一定要动手尝试，才能将所学知识应用于实际问题，强化理解。
...
关于 RAG 检索增强生成的详细学习，可以查看 [RAG 检索增强生成学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748766445023233)
...
RAG 现在挺火的，它本质上就是让大模型先查资料再回答问题，比直接训练模型要容易上手。你可以从了解它的基本概念开始，先思考一下为什么需要检索增强，不理解其中的技术细节也没关系，可以先从一个项目例子入手，比如让系统先从某个文档集合中找到相关内容，再将这些内容作为输入传递给生成模型，看看它能如何生成更准确的答案。在实践的过程中可以看看它是怎么运行的，了解下 RAG 的流程。然后可以着重理解RAG的应用场景和它在实际项目中的工作原理，再逐步深入学习如何调优检索部分和生成部分的协同工作，如何评估结果，并做性能优化，并且可以了解下里面用到的技术。
...
4）针对RAG系统中常见的幻觉问题，你会采用哪些分析和优化方案来减少其发生？
...
前面学习的都是RAG的理论知识，下面可以深入去实践RAG项目了，通过实际的开源项目，可以将理论知识应用于实践，深化对 RAG 技术的理解。通过阅读这些项目的源码，可以了解不同 RAG 框架的实现细节和设计思路。建议选择一个和自己目标最契合的项目，深入研究其架构和代码实现，并尝试进行本地部署和测试。
...
关于 LangChain 开发的详细学习，可以查看 [LangChain 开发学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748731967844354)
...
- LangChain的核心组件
  ...
  - 链（chain）
    - LCEL表达式
    - LCEL Runnable 协议设计和使用
    - LCEL 进阶使用：整合复杂逻辑的多链
    - LCEL添加记忆
    - LCEL中chain 和 prompt结合



##### 学习建议

学习 LangChain 框架对于大模型应用开发非常重要。它是一个开源框架，它简化了和大型语言模型（LLM）的集成，可以帮助你快速构建复杂的 AI 应用。你可以先了解 LangChain 的核心概念，如提示词管理、链的定义和管理等，这些是构建应用的基础。接着，深入学习 LangChain 的核心组件，包括 Chat Models 和 LLMs 的区别、模型 I/O 封装、数据连接、Memory 记忆封装以及链（Chain）的使用。这些组件是 LangChain 的精髓，掌握它们将使你能够灵活地构建和优化应用。学习过程中，不用死磕底层，先跑通几个Demo（比如本地知识库问答），再慢慢深入。学到能独立用LangChain接API、处理数据、搭出可用的AI工具就够用了，细节可以边做边补。
...
*   LlamaIndex 是什么
*   LlamaIndex 的优势和劣势
*   LlamaIndex 和 RAG 检索增强联合应用实践
*   LlamaIndex 和 LangChain 对比分析
...
关于 AI Agent 应用开发的详细学习，可以查看 [AI Agent 应用开发学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748661016997890)
...
4）吴恩达《深入模型量化|Quantization in Depth》
...
3）PTQ（训练后量化）和 QAT（量化感知训练）有什么区别？

4）量化技术对GPU芯片性能的影响有哪些？

5）如何选择适合的量化算法，PTQ 和 QAT 在不同场景下的适用性如何？
...
3）在评估大语言模型时，知识和能力的评估如何进行？有哪些常见的评估指标？
...
*   [什么是多模态模型](https://zh.wikipedia.org/wiki/%E5%A4%9A%E6%A8%A1%E6%80%81%E5%AD%A6%E4%B9%A0)。关于计算机视觉的详细学习，可以查看 [计算机视觉学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748555425394689)
*   多模态的应用场景
*   AIGC是什么
*   AIGC的应用场景
*   图像生成算法
...
9）吴恩达 x Open AI ChatGPT 提示工程教程
